...
<table class="messenger-list-item @if($user->id == $id && $id != "0") m-list-active @endif" data-contact="{{ $user->id }}">
    <tr data-action="0">
        ...
        <p data-id="{{ $type.'_'.$user->id }}">
            {{ strlen($user->firstname.' '.$user->surname) > 12 ? trim(substr($user->firstname.' '.$user->surname,0,24)).'..' : $user->firstname.' '.$user->surname }}
            <span>{{ $lastMessage ? $lastMessage->created_at->diffForHumans() : '' }}</span></p>
        <span>
            {{-- Last Message user indicator --}}
            {!!
                isset($lastMessage) && $lastMessage->from_id == Auth::user()->id
...
<table class="messenger-list-item" data-contact="{{ $user->id }}">
    <tr data-action="0">
        ...
        <td>
        <p data-id="{{ $type.'_'.$user->id }}">
            {{ strlen($user->surname) > 12 ? trim(substr($user->surname,0,12)).'..' : $user->surname }}
        </td>
